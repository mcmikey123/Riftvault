...
import { seedDb, TEST_USER_ID } from './fixtures.js';
...
describe('applyAdjustments', () => {
  ...
  it('clamps at zero and logs the applied delta, not the requested one', () => {
    const db = seedDb();
    applyAdjustments(db, TEST_USER_ID, [{ card_id: 'OGN-045', delta: 2 }], 'grid');
    const result = applyAdjustments(db, TEST_USER_ID, [{ card_id: 'OGN-045', delta: -5 }], 'grid');
  ...
  it('skips no-op log rows but still reports quantities', () => {
    const db = seedDb();
    const result = applyAdjustments(db, TEST_USER_ID, [{ card_id: 'OGN-045', delta: 0 }], 'grid');
    expect(result.rows[0]).toEqual({ card_id: 'OGN-045', qty: 0, qty_foil: 0 });
...
describe('undoBatch', () => {
  it('reverses a whole batch', () => {
    ...
    const undone = undoBatch(db, TEST_USER_ID, batch.batch_id);
  ...
  it('refuses to undo twice', () => {
    const db = seedDb();
    const batch = applyAdjustments(db, TEST_USER_ID, [{ card_id: 'OGN-045', delta: 3 }], 'scan');
    expect(undoBatch(db, TEST_USER_ID, batch.batch_id)).not.toBeNull();
    expect(undoBatch(db, TEST_USER_ID, batch.batch_id)).toBeNull();
  });

  it('returns null for unknown batches', () => {
    const db = seedDb();
    expect(undoBatch(db, TEST_USER_ID, 'nope')).toBeNull();
  });
});
