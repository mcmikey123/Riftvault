...
export function vaultRoutes(db: Db) {
  ...
  app.post('/vault/adjust', async (c) => {
    ...
    try {
      return c.json(applyAdjustments(db, adjustments, source, body.batch_id));
    } catch (err) {
  ...
  app.post('/vault/undo', (c) => {
    ...
    const result = undoBatch(db, batchId);
  ...
  app.get('/export', (c) => {
    const rows = db
      ...
      .all() as { set_code: string; collector_number: number; name: string; qty: number; qty_foil: number }[];
