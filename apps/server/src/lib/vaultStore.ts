...
export function applyAdjustments(
  ...
  const getRow = db.prepare(
    'SELECT qty, qty_foil FROM vault WHERE card_id = ?',
  );
  const cardExists = db.prepare('SELECT 1 FROM cards WHERE id = ?');
  const upsert = db.prepare(
    `INSERT INTO vault (card_id, qty, qty_foil, updated_at) VALUES (@card_id, @qty, @qty_foil, @now)
     ON CONFLICT(card_id) DO UPDATE SET qty = @qty, qty_foil = @qty_foil, updated_at = @now`,
  );
  const log = db.prepare(
    `INSERT INTO vault_log (card_id, delta, delta_foil, source, batch_id, created_at)
     VALUES (@card_id, @delta, @delta_foil, @source, @batch_id, @now)`,
  );
  ...
  const run = db.transaction(() => {
    for (const adj of adjustments) {
      ...
      const current = (getRow.get(adj.card_id) as { qty: number; qty_foil: number } | undefined) ?? {
        qty: 0,
        qty_foil: 0,
      };
      ...
      if (applied !== 0 || appliedFoil !== 0) {
        upsert.run({ card_id: adj.card_id, qty, qty_foil, now });
        log.run({
          card_id: adj.card_id,
          delta: applied,
...
/**
 * Reverse every log row of a batch. The undo itself is logged under batch
 * `undo:<original>` which doubles as the already-undone marker.
 */
export function undoBatch(db: Db, batchId: string): AdjustResult | null {
  const undoMarker = `undo:${batchId}`;
  const already = db
    .prepare('SELECT 1 FROM vault_log WHERE batch_id = ? LIMIT 1')
    .get(undoMarker);
  ...
  const rows = db
    .prepare(
      `SELECT card_id, SUM(delta) AS delta, SUM(delta_foil) AS delta_foil
       FROM vault_log WHERE batch_id = ? GROUP BY card_id`,
    )
    .all(batchId) as { card_id: string; delta: number; delta_foil: number }[];
